...
function clearPhoto() { selectedPhoto = null; photoInput.value = ''; photoPreview.classList.add('hidden'); photoLabel.classList.remove('has-photo'); photoLabel.textContent = '📷 Фото'; }
photoInput.addEventListener('change', () => { const f = photoInput.files[0]; if (f) { selectedPhoto = f; const r = new FileReader(); r.onload = (e) => { photoPreviewImg.src = e.target.result; photoPreview.classList.remove('hidden'); photoLabel.classList.add('has-photo'); photoLabel.textContent = '📷 Фото выбрано'; }; r.readAsDataURL(f); } });
...
async function loadFeed() {
    try {
        ...
        if (!posts || !posts.length) { feedContainer.innerHTML = '<div class="empty-feed"><div class="icon">🐣</div><p>Лента пуста.</p></div>'; return; }
        ...
        posts.forEach(post => {
            ...
            const del = (currentUser && String(post.userId) === String(currentUser.id)) ? `<button class="post-delete-btn" data-postid="${post.id}" title="Удалить">🗑️</button>` : '';
...
async function viewProfile(userId) {
    ...
    try {
        ...
        if (!posts || !posts.length) { pc.innerHTML = '<div class="empty-feed"><div class="icon">📭</div><p>У пользователя пока нет постов</p></div>'; }
        else {
            const ids = posts.map(p => p.id); const likesData = await apiCall('/api/likes', 'POST', { postIds: ids });
            posts.forEach(post => {
                const div = document.createElement('div'); div.className = 'post-card';
...
async function loadDialogs() {
    try { const dialogs = await apiCall('/api/dialogs', 'GET'); dialogsList.innerHTML = ''; if (!dialogs.length) { dialogsList.innerHTML = '<div class="no-dialogs">Нет диалогов</div>'; } dialogs.forEach(d => { const div = document.createElement('div'); div.className = 'dialog-item'; if (String(currentChatPartner) === String(d.userId)) div.classList.add('active'); const t = d.lastTime ? new Date(d.lastTime).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' }) : ''; div.innerHTML = `<div class="dialog-avatar">${d.avatarUrl ? `<img src="${d.avatarUrl}" style="width:100%;height:100%;border-radius:50%;object-fit:cover;">` : d.username.charAt(0).toUpperCase()}</div><div class="dialog-info"><div class="dialog-name">${escapeHTML(d.username)}</div><div class="dialog-last">${escapeHTML((d.lastMessage || '').substring(0, 30))}</div></div><div class="dialog-meta"><div class="dialog-time">${t}</div>${d.unread > 0 ? `<div class="unread-badge">${d.unread}</div>` : ''}</div>`; div.addEventListener('click', () => openChat(d.userId, d.username)); dialogsList.appendChild(div); }); } catch (err) {}
}
let st; searchUserInput.addEventListener('input', () => { clearTimeout(st); const q = searchUserInput.value.trim(); if (!q) { searchResults.classList.add('hidden'); return; } st = setTimeout(async () => { try { const users = await apiCall('/api/users/search?q=' + encodeURIComponent(q), 'GET'); searchResults.classList.remove('hidden'); searchResults.innerHTML = ''; if (!users.length) searchResults.innerHTML = '<div class="search-result-item" style="color:#999;">Никого нет</div>'; users.forEach(u => { const div = document.createElement('div'); div.className = 'search-result-item'; div.textContent = '👤 ' + u.username; div.addEventListener('click', () => { openChat(u.id, u.username); searchUserInput.value = ''; searchResults.classList.add('hidden'); }); searchResults.appendChild(div); }); } catch (err) {} }, 300); });
function openChat(userId, username) { currentChatPartner = String(userId); chatPartnerText.textContent = '💬 ' + username; messageInput.disabled = false; sendMessageBtn.disabled = false; loadMessages(); loadDialogs(); if (window.innerWidth <= 768) { dialogsSidebar.classList.add('chat-open'); messagesLayout.classList.add('mobile-view'); } }
chatBackBtn.addEventListener('click', () => { dialogsSidebar.classList.remove('chat-open'); messagesLayout.classList.remove('mobile-view'); currentChatPartner = null; chatPartnerText.textContent = 'Выберите диалог'; messageInput.disabled = true; sendMessageBtn.disabled = true; chatMessages.innerHTML = '<div class="chat-empty">Выберите диалог или найдите пользователя</div>'; });
async function loadMessages() { if (!currentChatPartner) { chatMessages.innerHTML = '<div class="chat-empty">Выберите диалог</div>'; return; } try { const msgs = await apiCall('/api/messages/' + currentChatPartner, 'GET'); chatMessages.innerHTML = ''; if (!msgs.length) chatMessages.innerHTML = '<div class="chat-empty">Напишите первым! 👋</div>'; msgs.forEach(m => { const div = document.createElement('div'); div.className = 'message ' + (String(m.from) === String(currentUser.id) ? 'message-sent' : 'message-received'); const t = new Date(m.time).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' }); div.innerHTML = escapeHTML(m.text) + '<div class="message-time">' + t + '</div>'; chatMessages.appendChild(div); }); chatMessages.scrollTop = chatMessages.scrollHeight; updateUnreadBadge(); } catch (err) {} }
