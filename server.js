...
async function initDb() {
    ...
    setDb(db);
    setSaveDb(saveDb);
    console.log('✅ БД инициализирована и передана в WebSocket');
...
const server = http.createServer(async (req, res) => {
    ...
    if (url === '/api/post' && method === 'POST') {
        ...
        if (queryOne('SELECT COUNT(*) as count FROM posts WHERE userId = ? AND date = ?', [currentUser.id, today]).count > 0) 
            return serveJSON(res, { error: 'Вы уже публиковали сегодня' }, 400);
        ...
        runSql('INSERT INTO posts (id, userId, content, imageUrl, date, time) VALUES (?, ?, ?, ?, ?, ?)', 
            [postId, currentUser.id, sanitizedContent, imageUrl, today, new Date().toISOString()]);
    ...
    if (url === '/api/like' && method === 'POST') {
        if (!currentUser) return serveJSON(res, { error: 'Не авторизован' }, 401);
        if (!checkRateLimit('like_' + currentUser.id, 10, 1000)) {
            return serveJSON(res, { error: 'Слишком быстро' }, 429);
        }
    ...
    if (url === '/api/likes' && method === 'POST') {
        ...
        postIds.forEach(pid => { 
            result[pid] = { 
                count: queryOne('SELECT COUNT(*) as count FROM likes WHERE postId = ?', [pid]).count, 
                liked: myLikes.includes(pid) 
            }; 
        });
    ...
    if (url.match(/^\/api\/user\/[^/]+$/) && method === 'GET') {
        ...
        return serveJSON(res, { 
            ...
            createdAt: user.createdAt, totalPosts: posts.length, streak, 
            online: isOnline(user.lastSeen), lastSeen: user.lastSeen 
        });
    }
    ...
    if (url.match(/^\/api\/user\/[^/]+\/posts$/) && method === 'GET') {
        ...
        const user = queryOne('SELECT * FROM users WHERE id = ?', [userId]);
        if (!user) return serveJSON(res, { error: 'Пользователь не найден' }, 404);
        const posts = queryAll('SELECT * FROM posts WHERE userId = ? ORDER BY time DESC', [userId]).map(p => ({ 
            ...p, author: user.username, authorAvatar: user.avatarUrl 
        }));
        return serveJSON(res, posts);
    }
    ...
    if (url.startsWith('/api/users/search') && method === 'GET') {
        ...
        return serveJSON(res, filtered.map(u => ({ 
            id: String(u.id), username: u.username, avatarUrl: u.avatarUrl, 
    ...
    if (url === '/api/dialogs' && method === 'GET') {
        ...
        for (const p of partners) {
            ...
            dialogs.push({ 
                ...
                lastMessage: lastMsg ? parseLastMsg(lastMsg.text) : '', 
                lastTime: lastMsg ? lastMsg.time : '', 
                unread, online: isOnline(partner.lastSeen) 
            });
        }
    ...
    if (url === '/api/messages' && method === 'POST') {
        ...
        sendMessageViaWS(currentUser.id, to, {
            type: 'new_message',
            payload: {
                id: msgId,
    ...
    if (url === '/api/messages/photo' && method === 'POST') {
        ...
        const msgId = Date.now().toString() + '-' + crypto.randomBytes(4).toString('hex');
        const sanitizedText = sanitizeText(text, 2000);
        const storedText = JSON.stringify({ text: sanitizedText, imageUrl, audioUrl: null, duration: null, replyTo, replyToText });
        runSql('INSERT INTO messages (id, fromUserId, toUserId, text, time, read) VALUES (?, ?, ?, ?, ?, 0)', [msgId, currentUser.id, String(to), storedText, new Date().toISOString()]);
        ...
        if (contentType.includes('multipart/form-data')) {
            ...
            if (files.cover && files.cover[0]) {
                ...
                if (currentUser.coverUrl) { 
                    const oldPath = path.join(__dirname, 'public', currentUser.coverUrl); 
                    if (fs.existsSync(oldPath)) fs.unlinkSync(oldPath); 
                }
                fs.renameSync(file.filepath, path.join(UPLOADS_DIR, fileName));
